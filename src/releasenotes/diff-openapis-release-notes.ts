...
function compareObjects(previousNode: any, nextNode: any, currentPath: string, artifactType: "openapi" | "asyncapi", changes: ReleaseNoteChange[]): void {
  ...
  for (const key of nextKeys) {
    ...
    if (!(key in (previousNode || {}))) {
      changes.push(createChange({
        ...
        element: key,
        path: nextPath,
        category: currentPath.includes(".paths") ? "path-added" : currentPath.includes(".channels") ? "channel-added" : "unclassified-change",
        message: `Added '${key}'.`,
        breaking: false,
      }));
      continue;
    ...
    if (key === "required" && typeof previousValue === "boolean" && typeof nextValue === "boolean" && previousValue !== nextValue) {
      changes.push(createChange({
        artifactType,
        element: currentPath.split(".").at(-1) || key,
        path: nextPath,
        category: nextValue ? "became-required" : "no-longer-required",
        message: nextValue ? "Element is now required." : "Element is no longer required.",
        breaking: nextValue,
      }));
      continue;
    }
    ...
    if (isObject(previousValue) && isObject(nextValue)) {
      compareObjects(previousValue, nextValue, nextPath, artifactType, changes);
      continue;
    }
  ...
  for (const key of previousKeys) {
    if (!(key in (nextNode || {}))) {
      changes.push(createChange({
        ...
        category: currentPath.includes(".paths") ? "path-removed" : currentPath.includes(".channels") ? "channel-removed" : "unclassified-change",
        message: `Removed '${key}'.`,
        breaking: true,
      }));
    }
...
export async function generateOpenApiReleaseNotes(artifactName: string, fromVersion: string, toVersion: string, previousYamlPath: string, nextYamlPath: string): Promise<ReleaseNotes> {
  ...
  compareObjects(previousDocument, nextDocument, "$", "openapi", changes);
  return { artifactType:"openapi", artifactName, fromVersion, toVersion, generatedAt:new Date().toISOString(), changes };
}
